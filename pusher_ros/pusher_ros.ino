...
#include <ros.h>
#include <whiff_waff_msgs/HopperGoal.h>
...
whiff_waff_msgs::HopperGoal active_goal;
...
void messageCb( const whiff_waff_msgs::HopperGoal& goal_msg){
  active_goal = goal_msg;
  if( active_goal.action == whiff_waff_msgs::HopperGoal::ACTION_OFF ){
    servo1.detach();
  }else{
    if(!servo1.attached()) servo1.attach(9);

    switch( active_goal.action ){
    case whiff_waff_msgs::HopperGoal::ACTION_LOCK:
      target_servo_position = POS_LOCK_ONLY;
      break;
    case whiff_waff_msgs::HopperGoal::ACTION_OPEN:
      target_servo_position = POS_FULL_OPEN;
      break;
    case whiff_waff_msgs::HopperGoal::ACTION_SHOOT:
...
ros::Subscriber<whiff_waff_msgs::HopperGoal> goal_sub("hopper_goal", &messageCb );

void setup() {

  AFMS.begin();  // create with the default frequency 1.6KHz

  active_goal.action = whiff_waff_msgs::HopperGoal::ACTION_OFF;
  servo_position=0;
...
void loop() {
  ...
  if( active_goal.action == whiff_waff_msgs::HopperGoal::ACTION_SHOOT && servo_position == target_servo_position ){
